...
namespace Opsive.UltimateCharacterController.Integrations.UltimateInventorySystem
{
    ...
    /// <summary>
    /// The Item Definition Item Set Rule define a rules which creates item sets for matching item definition in each slot.
    /// </summary>
    [CreateAssetMenu(menuName = "Opsive/Ultimate Character Controller/Inventory/Item Definition Item Set Rule", fileName = "MyItemDefinitionItemSetRule", order = 0)]
    [Serializable]
    public class ItemDefinitionItemSetRule : InventorySystemItemSetRule
    {
        [Tooltip("The Item Definitions that occupy the inventory slots.")]
        [SerializeField] protected DynamicItemDefinitionArray m_ItemDefinitionSlots;

        protected Dictionary<IItemIdentifier, int> m_CachedItemAmount;
        
        [Shared.Utility.NonSerialized] public ItemDefinition[] ItemDefinitionSlots { get { return m_ItemDefinitionSlots; } set { m_ItemDefinitionSlots = value; } }
            ...
            ListSlice<IItemIdentifier> currentPermutation, CharacterItem characterItem)
        {
            ...
            var itemDefinitionSlots = m_ItemDefinitionSlots.Value;
            
            if (slotID < 0 || slotID >= itemDefinitionSlots.Length) {
                return false;
            }
            
            if (itemDefinitionSlots[slotID] == null) {
                return false;
            }
            if (itemDefinitionSlots[slotID] != null && itemDefinitionSlots[slotID].InherentlyContains(item) == false) {
                return false;
            }
        ...
        protected override bool CanSlotBeNull(int slotID)
        {
            ...
            return itemDefinitionSlots[slotID] == null;
        }
        ...
        {
            if (m_DoNotShareItemBetweenSet && SharesItemBetweenSet(itemSet)) {
                return false;
            }
